...
public:
    ...
    //-----------------------------------------------------
    feature_count_type
    remove_features_with_more_locations_than_distributed(std::uint32_t overpopulated_keys[], std::uint32_t num_overpopulated_keys)
    {
        //note that features are not really removed, because the hashmap
        //does not support erasing keys; instead all values belonging to
        //the key are cleared and the key is kept without values
        ...
        for(std::uint32_t i = 0; i< num_overpopulated_keys; ++i) {

            auto current = features_.find(overpopulated_keys[i]);
            if (current != features_.end()) {
                features_.clear(current);
                ++rem;
            }
                    ...
                    file_source source = file_source{})
    {
        ...
        //---------------------------------------------------------------
        bool add_target_distributed(const sequence& seq, taxon_name sid,
                        taxon_id parentTaxid = 0,
                        file_source source = file_source{}, int num_procs = 1, int my_id = 0)
        {
            using std::begin;
            using std::end;
            ...
            //sketch sequence -> insert features
            if ((targetCount % num_procs) == (unsigned long)my_id) {
                source.windows = add_all_window_sketches(seq, targetCount);
            }
    ...
    std::uint32_t *get_keys_num_items(std::uint32_t *items) {
